...
public:
    ...
    future<stop_iteration> handle_do_put_ssl_err(const int ssl_err) {
        ...
        case SSL_ERROR_WANT_WRITE:
            ...
            return maybe_perform_push_with_wait().then([this](bool) {
                // The _out_sem semaphore is currently being held, however if we need
                // to receive data, we need to obtain a lock on the _in_sem semaphore
    ...
    // Called after locking the _out_sem semaphore
    // This fuction will attempt to write user (unencrypted)
    // data into the SSL session using SSL_write_ex.
    // Once the data is written, this function will call
    // perform_push() and wait for the data to be sent.
    ...
    future<> put(net::packet p) override {
        ...
        return with_semaphore(
                 _out_sem, 1, std::bind(&session::do_put, this, i, e))
    ...
    future<buf_type> do_get() {
        ...
        return f.then([this] {
            ...
            if (read_result != 1) {
                ...
                case SSL_ERROR_WANT_READ:
                case SSL_ERROR_WANT_WRITE:
                    // The _in_sem semaphore is being held at this point, but if we need to send data,
                    // then we need to obtain a lock on the _out_sem semaphore
                    return with_semaphore(_out_sem, 1, [this] {
                        return maybe_perform_push_with_wait();
                    }).then([this](bool) {
                        return perform_pull().then([this] { return do_get(); });
                    });
                case SSL_ERROR_SSL:
                    {
                        auto ec = ERR_GET_REASON(ERR_peek_error());
...
private:
    ...
    // This function is used put data into the _in_bio.  It will
